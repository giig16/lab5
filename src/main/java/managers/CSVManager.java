...
public class CSVManager extends FileManager {
    ...
    /** Метод чтения коллекции из CSV-файла */
    public LinkedHashSet<City> readCollectionFromFile() {
        ...
        try (CSVReader reader = new CSVReader(new InputStreamReader(new FileInputStream(filePath), StandardCharsets.UTF_8))) {
            ...
            for (String[] columns : lines) {
                ...
                String owner = columns.length > 12 ? columns[12] : "admin"; // если файла нет, то дефолтный владелец "admin"

                City city = new City(name, coordinates, creationDate, area, population, metersAboveSeaLevel, establishmentDate, government, standardOfLiving, governor, owner);
    ...
    /** Метод записи коллекции в CSV-файл */
        ...
        try (CSVWriter writer = new CSVWriter(new OutputStreamWriter(new BufferedOutputStream(new FileOutputStream(filePath, false)), StandardCharsets.UTF_8))) {
            for (City city : cities) {
                String[] line = new String[]{
                        ...
                        city.getOwner() // <-- теперь пишем владельца
                };
